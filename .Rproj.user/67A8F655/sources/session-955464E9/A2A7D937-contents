...
#' @param DistRob One of "Cook", "CookEmp" or "LikelyEmp"; removes outlying heterozygous samples anyway, either by checking whether the difference of their leave-one-out maximum
...
#' @param CookMargin Number. See \code{DistRob}.
#' @param LikEmpNum Integer. See \code{DistRob}.
#' @param LikMargin Integer. See \code{DistRob}.
#' @param NumHetMin Integer. Minimal amount of expected heterozygotes required to even try and perform outlier-removal on these (default 5).
#' @param MaxOutFrac Number. Maximum allowed fraction of detected outlying samples; if more, the outlier-detection procedure is disregarded (default 0.5)
...
EMfit_betabinom_robust <- function(data_counts, allelefreq=0.5, SE, inbr = 0, dltaco = 10^-6, HWE = FALSE, p_InitEst = FALSE, 
  ...
  if(DistRob == "Cook" | DistRob == "CookEmp" | DistRob == "LikelyEmp"){
    TussenDF <- data.frame("pvv" = spv, "prr" = spr, "prv" = sprv)
    genotypeN <- c("rr", "rv", "vv")[apply(TussenDF[, c("prr", "prv", "pvv")], 1, function(x) which(x==max(x))[1])]
    
    if(sum(genotypeN=="rv") < NumHetMin){
      RobFlag <- "NotEnoughHet"
      data_counts$Outlier <- 0
    }else{
      DistsRes <- LikelyDistsHet(ref_counts=data_counts$ref_count, var_counts=data_counts$var_count, 
                                 sprv=sprv, parvec_cur=c(probshift, theta_het), NoSplitHet=NoSplitHet, ResetThetaMin=ResetThetaMin, 
                                 ResetThetaMax=ResetThetaMax, SE=SE)
      
      if(DistRob == "Cook"){
        PiDists <- DistsRes$PiDists
        ThetaDists <- DistsRes$ThetaDists
        PiDistsHet <- PiDists[genotypeN=="rv"]
        ThetaDistsHet <- ThetaDists[genotypeN=="rv"]
        OutlierPi <- which(PiDists < mean(PiDistsHet)-CookMargin*sd(PiDistsHet) | PiDists > mean(PiDistsHet)+CookMargin*sd(PiDistsHet))
        OutlierTheta <- which(ThetaDists < mean(ThetaDistsHet)-CookMargin*sd(ThetaDistsHet) | ThetaDists > mean(ThetaDistsHet)+CookMargin*sd(ThetaDistsHet))
        Outliers <- union(OutlierPi, OutlierTheta)
      }else if(DistRob == "CookEmp"){
        HetCounts <- (data_counts$ref_count + data_counts$var_count)[genotypeN == "rv"]
        MP <- ceiling(100/length(HetCounts))
        vecPi <- c()
        vecTheta <- c()
        for(i in 1:MP){
          RandoRef <- rBetaBinom(HetCounts, pi=probshift, theta=theta_het)
          RandoVar <- HetCounts - RandoRef
          RandoDists <- LikelyDistsHet(ref_counts=RandoRef, var_counts=RandoVar, 
                                       sprv=rep(1, length(RandoRef)), parvec_cur=c(probshift, theta_het), NoSplitHet=NoSplitHet, ResetThetaMin=ResetThetaMin,
                                       ResetThetaMax=ResetThetaMax, SE=SE,
                                       ReEstPars = TRUE)
          PiDistsRando <- RandoDists$PiDists
          ThetaDistsRando <- RandoDists$ThetaDists
          vecPi <- c(vecPi, PiDistsRando)
          vecTheta <- c(vecTheta, ThetaDistsRando)
        }
        PiDists <- DistsRes$PiDists
        ThetaDists <- DistsRes$ThetaDists
        PiDistsHet <- PiDists[genotypeN=="rv"]
        ThetaDistsHet <- ThetaDists[genotypeN=="rv"]
        OutlierPi <- which(PiDists < mean(PiDistsHet)-CookMargin*sd(vecPi) | PiDists > mean(PiDistsHet)+CookMargin*sd(vecPi))
        OutlierTheta <- which(ThetaDists < mean(ThetaDistsHet)-CookMargin*sd(vecTheta) | ThetaDists > mean(ThetaDistsHet)+CookMargin*sd(vecTheta))
        Outliers <- union(OutlierPi, OutlierTheta)
      }else{
        HetCounts <- (data_counts$ref_count + data_counts$var_count)[genotypeN == "rv"]
        ML <- ceiling(LikEmpNum/length(HetCounts))
        vecLik <- c()
        for(i in 1:ML){
          RandoRef <- rBetaBinom(HetCounts, pi=probshift, theta=theta_het)
          RandoVar <- HetCounts - RandoRef
          RandoDists <- LikelyDistsHet(ref_counts=RandoRef, var_counts=RandoVar, 
                                       ...
                                       ResetThetaMax=ResetThetaMax, SE=SE,
                                       ReEstPars = TRUE)
          LikDistRando <- RandoDists$LikDists
          vecLik <- c(vecLik, LikDistRando)
        }
        LikDists <- DistsRes$LikDists
        CritVal <- sort(vecLik, decreasing = TRUE)[LikMargin+1]
        Outliers <- which(LikDists > CritVal)
      }
      ...
      if(length(Outliers)==0){
        RobFlag <- "NoOutliers"
        data_counts$Outlier <- 0
      }else if (length(Outliers)/sum(genotypeN=="rv") > MaxOutFrac){
        RobFlag <- "TooManyOutliers"
        data_counts$Outlier <- 0
      }else{
        ...
        if(fitH0){
          ...
          data_counts$Outlier <- 0
          data_counts$Outlier[Outliers] <- 1
          ...
          dataclone <- NewRes$data_hash
          spr_H0 <- spv_H0 <- sprv_H0 <- rep(NA, nrow(data_counts))
          spr_H0[-Outliers] <- dataclone$prr_H0
          spv_H0[-Outliers] <- dataclone$pvv_H0
          sprv_H0[-Outliers] <- dataclone$prv_H0
          ...
          NewRes <- EMfit_betabinom(data_counts[-Outliers,], allelefreq=allelefreq, SE=SE, inbr=inbr, dltaco=dltaco, HWE=HWE, p_InitEst=p_InitEst,
                                       ThetaInits=ThetaInits, ReEstThetas=ReEstThetas, NoSplitHom=NoSplitHom, NoSplitHet=NoSplitHet,
                                       ResetThetaMin=ResetThetaMin, ResetThetaMax=ResetThetaMax, fitH0=fitH0)
          probshift <- NewRes$AB
          theta_het <- NewRes$theta_het
          theta_hom <- NewRes$theta_hom
  ...
  if(!fitH0){
    results <- list(AB = probshift, nrep = nrep, quality = quality, rho_rr = pr, rho_vv = pv, rho_rv = prv, data_hash = data_counts, 
                    theta_hom = theta_hom, theta_het = theta_het, RobFlag = RobFlag)
    return(results)
  }
  ...
  while (dlta > dltaco & nrep2 < 100) {
    ...
    if(ReEstThetas == "moment"){
      ...
      if(is.nan(theta_het_H0) | is.infinite(theta_het_H0)){
        theta_het_H0 <- ResetThetaMin
      }
      if(is.nan(theta_hom_H0) | is.infinite(theta_hom_H0)){
  ...
  return(results)
    ...
    #NewRes$rho_rv <- prv
    #NewRes$rho_vv <- pv
    if(NewRes$quality != "!" & quality == "!"){
      NewRes$quality <- "!"
    }
    NewRes$data_hash <- data_counts
    NewRes$RobFlag <- RobFlag
    return(NewRes)
